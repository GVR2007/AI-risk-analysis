#!/usr/bin/env python3
"""
ABUSE-RING SENTINEL - VERSION 12 (Regularized Structural Sentinel)
==================================================================
Upgrades:
    1. Regularized LightGBM: Prevents overfitting on high-dimensional structural features using max_depth=6 and subsampling.
    2. Optimized Cost Grid Search: Restores wide-band threshold optimization to capture optimal financial trade-offs.
"""
...
MODEL_FILE = "ieee_abuse_ring_sentinel_v12.pkl"
...
# =====================================================================
# REGULARIZED MODELING & CAPACITY MANAGEMENT
# =====================================================================
...
def create_regularized_model():
    return LGBMClassifier(
        n_estimators=500,
        learning_rate=0.02,
        max_depth=6,             # Strict depth restriction to prevent overfitting
        num_leaves=31,           # Standard regularized leaf count
        min_child_samples=50,    # Prevents leaf isolation
...
def find_best_threshold_grid_search(y_val, probability, transaction_amount, features_df):
    candidate_thresholds = np.concatenate([
        np.linspace(0.05, 0.15, 30),  # Tighter lower bound to eliminate excess FPs
        np.linspace(0.15, 0.85, 40)
    ])
    best_cost = float("inf")
    best_thresh = 0.5
...
def run_pipeline():
    print(f"\n{'=' * 70}\nABUSE-RING SENTINEL V12 (REGULARIZED STRUCTURAL)\n{'=' * 70}")
