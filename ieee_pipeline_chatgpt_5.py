#!/usr/bin/env python3
"""
ABUSE-RING SENTINEL - VERSION 5
==============================
Upgrades:
    1. Tuned Class Weighting: scale_pos_weight=5 to protect precision.
    2. Graph Centrality: Degree centrality and connected component size.
    3. Amount-Aware Risk Tiering: Dynamic review policy based on tx size.
    4. Pure Cost Grid Search: Direct minimization of financial loss.
"""
...
MODEL_FILE = "ieee_abuse_ring_sentinel_v5.pkl"
...
# =====================================================================
# GRAPH CENTRALITY & TOPOLOGY FEATURES (NEW)
# =====================================================================
# def add_graph_centrality_features(df):
#     df = df.copy()
#     G = nx.Graph()
    
#     # Construct lightweight interaction graph per split
#     for _, row in df.iterrows():
#         dev, card, addr = row["_device"], row["_card"], row["_address"]
#         if dev != "__MISSING__":
#             if card != "__MISSING__":
#                 G.add_edge(f"dev_{dev}", f"card_{card}")
#             if addr != "__MISSING__":
#                 G.add_edge(f"dev_{dev}", f"addr_{addr}")

#     # Compute graph metrics
#     try:
#         deg_cent = nx.degree_centrality(G)
#         components = list(nx.connected_components(G))
#         comp_map = {node: len(c) for c in components for node in c}
#     except Exception:
#         deg_cent, comp_map = {}, {}

#     def get_deg(val, prefix):
#         return deg_cent.get(f"{prefix}_{val}", 0.0)

#     def get_comp(val, prefix):
#         return comp_map.get(f"{prefix}_{val}", 1)

#     df["device_degree_centrality"] = df["_device"].apply(lambda x: get_deg(x, "dev"))
#     df["card_degree_centrality"] = df["_card"].apply(lambda x: get_deg(x, "card"))
#     df["component_size"] = df["_device"].apply(lambda x: get_comp(x, "dev")).astype(float)
...
def add_graph_centrality_features(df):
    ...
    # Vectorized degree and component size calculation using Pandas instead of NetworkX
    # This avoids heavy graph object overhead in RAM
    ...
    # Approximate component size using unique interconnected footprint
...
# =====================================================================
# MODELING, AMOUNT-AWARE TIERING & GRID SEARCH
# =====================================================================
...
def create_model():
    return LGBMClassifier(
        ...
        max_depth=9,
        num_leaves=128,
        scale_pos_weight=5,  # Tuned down to protect precision
...
def calculate_cost_with_tiering(y_true, y_pred, transaction_amount, ring_scores=None):
    y_true = np.asarray(y_true, dtype=int)
    y_pred = np.asarray(y_pred, dtype=int)
    amount = np.asarray(transaction_amount, dtype=float)

    # Amount-Aware Risk Tiering Policy
    if ring_scores is not None:
        scores = np.asarray(ring_scores, dtype=float)
        # Low value (<500): Only flag if ring score is extreme (>0.85)
        low_val = amount < 500
        y_pred = np.where(low_val & (scores < 0.85), 0, y_pred)
        # High value (>5000): Lower barrier
        high_val = amount > 5000
        y_pred = np.where(high_val & (scores > 0.30), 1, y_pred)
    ...
    return {
        ...
        "total_cost": fp_cost + fn_exposure, "adjusted_preds": y_pred
    }


def find_best_threshold_grid_search(y_val, probability, transaction_amount, ring_scores):
    candidate_thresholds = np.linspace(0.05, 0.95, 100)
    best_cost = float("inf")
    best_thresh = 0.5

    for thresh in candidate_thresholds:
        preds = (probability >= thresh).astype(int)
        res = calculate_cost_with_tiering(y_val, preds, transaction_amount, ring_scores)
...
def evaluate(model, X_test, y_test, amount_test, ring_scores, threshold, name):
    probability = model.predict_proba(X_test)[:, 1]
    prediction = (probability >= threshold).astype(int)

    cost_res = calculate_cost_with_tiering(y_test, prediction, amount_test, ring_scores)
...
def run_pipeline():
    ...
    thresh_h = find_best_threshold_grid_search(y_val, val_probs_h, val_amt, val_scores)
    ...
    res_h = evaluate(model_h, X_te_h, y_test, test_amt, test_scores, thresh_h, "2. HYBRID SENTINEL")
