#!/usr/bin/env python3
"""
ABUSE-RING SENTINEL - VERSION 6 (Precision-Optimized)
===================================================
Upgrades:
    1. Tuned Class Weighting: scale_pos_weight=3 to prevent over-flagging.
    2. Precision Floor: Enforces >= 12% precision in grid search optimization.
    3. Two-Stage Hard Ring Filter: Requires multi-card/component size evidence.
    4. Interaction Features: Added ring density and velocity interaction metrics.
"""
...
MODEL_FILE = "ieee_abuse_ring_sentinel_v6.pkl"
...
# =====================================================================
# MODELING, HARD RING FILTER & PRECISION FLOOR GRID SEARCH
# =====================================================================
...
def create_model():
    return LGBMClassifier(
        ...
        max_depth=9,
        num_leaves=128,
        scale_pos_weight=3,  # Tuned down to protect precision
...
def calculate_cost_with_hard_filter(y_true, y_pred, transaction_amount, ring_scores=None, features_df=None):
    y_true = np.asarray(y_true, dtype=int)
    y_pred = np.asarray(y_pred, dtype=int)
    amount = np.asarray(transaction_amount, dtype=float)

    # Two-Stage Hard Ring Filter (NEW): Must have model prediction AND physical ring evidence
    if features_df is not None and "device_unique_cards" in features_df.columns:
        ...
        # Hard constraint: Alert requires explicit multi-card coordination or cluster size
        hard_ring_mask = (dev_cards >= 3) | (comp_size >= 3)
        y_pred = y_pred & hard_ring_mask
...
def find_best_threshold_with_precision_floor(y_val, probability, transaction_amount, ring_scores, features_df):
    ...
    for thresh in candidate_thresholds:
        ...
        res = calculate_cost_with_hard_filter(y_val, preds, transaction_amount, ring_scores, features_df)
        
        # Calculate precision for this threshold iteration
        tp = ((res["adjusted_preds"] == 1) & (y_val == 1)).sum()
        fp = ((res["adjusted_preds"] == 1) & (y_val == 0)).sum()
        prec = tp / (tp + fp) if (tp + fp) > 0 else 0.0

        # Enforce Minimum Precision Floor (>= 12%)
        if prec >= 0.12 and res["total_cost"] < best_cost:
            best_cost = res["total_cost"]
            best_thresh = thresh

    # Fallback if precision floor is too restrictive for validation data
...
def evaluate(model, X_test, y_test, amount_test, ring_scores, features_df, threshold, name):
    probability = model.predict_proba(X_test)[:, 1]
    prediction = (probability >= threshold).astype(int)

    cost_res = calculate_cost_with_hard_filter(y_test, prediction, amount_test, ring_scores, features_df)
...
def run_pipeline():
    print(f"\n{'=' * 70}\nABUSE-RING SENTINEL V6 (PRECISION-OPTIMIZED)\n{'=' * 70}")
    ...
    thresh_b = find_best_threshold_with_precision_floor(y_val, val_probs_b, val_amt, None, validation_features)
    ...
    thresh_h = find_best_threshold_with_precision_floor(y_val, val_probs_h, val_amt, val_scores, validation_features)

    test_scores = test_features["abuse_ring_score"].to_numpy()
    res_b = evaluate(model_b, X_te_b, y_test, test_amt, None, test_features, thresh_b, "1. BASELINE MODEL")
    res_h = evaluate(model_h, X_te_h, y_test, test_amt, test_scores, test_features, thresh_h, "2. HYBRID SENTINEL")
