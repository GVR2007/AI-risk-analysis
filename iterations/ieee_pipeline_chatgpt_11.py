#!/usr/bin/env python3
"""
ABUSE-RING SENTINEL - VERSION 11 (True Structural Sentinel)
===========================================================
Upgrades:
    1. Removes Target Leakage: Completely strips out historical fraud rate features so the model relies purely on network topology.
    2. Enforced Capacity Budget: Tunes the threshold search to strictly respect the 8,000-review operational cap.
"""
...
MODEL_FILE = "ieee_abuse_ring_sentinel_v11.pkl"
...
USD_TO_INR = 83.00
MAX_MANUAL_REVIEWS_CAP = 8000

BASELINE_FEATURES = [
    "TransactionAmt", "card2", "card3", "card5", "addr2", "dist1", "C1", "C2", "C3", "C4", "C5",
]

# Pure structural network and velocity features (No historical target leakage)
...
def calculate_cost_with_capacity_constraint(y_true, probability, transaction_amount, threshold, features_df=None):
    ...
    if total_alerts > MAX_MANUAL_REVIEWS_CAP:
        ...
        capacity_penalty = excess_alerts * 800.0  # Strict enforcement penalty for exceeding operational capacity
...
#     candidate_thresholds = np.linspace(0.15, 0.85, 70) # Restrict search to higher thresholds to respect capacity cap
#     best_cost = float("inf")
#     best_thresh = 0.5
...
def find_best_threshold_grid_search(y_val, probability, transaction_amount, features_df):
    # Widen the search range to capture optimal lower thresholds without choking recall
    candidate_thresholds = np.concatenate([
        np.linspace(0.03, 0.15, 40),
...
def print_feature_importance(model, feature_names, top_n=15):
    ...
    print(f"\n{'=' * 70}\nTOP STRUCTURAL SENTINEL FEATURE IMPORTANCES (GAIN)\n{'=' * 70}")
...
def run_pipeline():
    print(f"\n{'=' * 70}\nABUSE-RING SENTINEL V11 (TRUE STRUCTURAL)\n{'=' * 70}")
    ...
    # Hybrid Sentinel
    print("Training Structural Hybrid Sentinel...")
    ...
    res_h = evaluate(model_h, X_te_h, y_test, test_amt, test_features, thresh_h, "2. STRUCTURAL HYBRID SENTINEL")
