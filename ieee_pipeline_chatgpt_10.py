#!/usr/bin/env python3
"""
ABUSE-RING SENTINEL - VERSION 10 (Capacity-Constrained & Audited)
=================================================================
Upgrades:
    1. Operational Capacity Constraint: Imposes a strict review budget (max 8,000 alerts) to prevent false-positive flooding.
    2. Fixed Feature Alignment: Resolves column-length mismatch during feature importance extraction.
    3. Leakage & Overfitting Audit: Retains train-test divergence checks and feature gain inspection.
"""
...
MODEL_FILE = "ieee_abuse_ring_sentinel_v10.pkl"
...
USD_TO_INR = 83.00
MAX_MANUAL_REVIEWS_CAP = 8000  # Operational capacity constraint per batch
...
# =====================================================================
# MODELING, CAPACITY CONSTRAINT & AUDITING
# =====================================================================
...
def calculate_cost_with_capacity_constraint(y_true, probability, transaction_amount, threshold, features_df=None):
    ...
    # Operational Capacity Constraint: If total alerts exceed MAX_MANUAL_REVIEWS_CAP, 
    # impose a steep penalty on excess alerts to prevent infinite review inflation.
    total_alerts = int(y_pred.sum())
    capacity_penalty = 0.0
    if total_alerts > MAX_MANUAL_REVIEWS_CAP:
        excess_alerts = total_alerts - MAX_MANUAL_REVIEWS_CAP
        capacity_penalty = excess_alerts * 500.0  # Heavy operational friction penalty per overflow review
    ...
    return {
        ...
        "total_cost": fp_cost + fn_exposure + capacity_penalty, "adjusted_preds": y_pred, "adjusted_probs": prob
    }
...
def print_feature_importance(model, feature_names, top_n=15):
    ...
    print(f"\n{'=' * 70}\nTOP HYBRID SENTINEL FEATURE IMPORTANCES (GAIN)\n{'=' * 70}")
    for _, row in importance.iterrows():
        print(f"{row['feature']:<40} {row['importance']:.2f}")


def evaluate_with_validation_check(model, X_val, y_val, val_amt, val_scores, X_test, y_test, test_amt, test_scores, threshold, features_val, features_test):
    val_res = calculate_cost_with_capacity_constraint(y_val, model.predict_proba(X_val)[:, 1], val_amt, threshold, features_val)
    val_recall = recall_score(y_val, val_res["adjusted_preds"], zero_division=0)
    
    test_res = calculate_cost_with_capacity_constraint(y_test, model.predict_proba(X_test)[:, 1], test_amt, threshold, features_test)
    test_recall = recall_score(y_test, test_res["adjusted_preds"], zero_division=0)

    print(f"\n{'=' * 70}\nVALIDATION VS TEST OVERFIT CHECK\n{'=' * 70}")
    print(f"Validation Recall: {val_recall:.4f} | Validation Cost: ₹{val_res['total_cost']:,.2f}")
    print(f"Test Recall:       {test_recall:.4f} | Test Cost:       ₹{test_res['total_cost']:,.2f}")
    
    if abs(val_recall - test_recall) > 0.05:
        print("STATUS: WARNING - Significant recall divergence detected between splits.")
    else:
        print("STATUS: PASSED - Validation and test performance metrics are well-aligned.")


def audit_operational_workload(res_h, total_test_samples):
    fp_count = res_h["fp"]
    review_workload_pct = (fp_count / total_test_samples) * 100

    print(f"\n{'=' * 70}\nOPERATIONAL CAPACITY AUDIT\n{'=' * 70}")
    print(f"Total Test Transactions: {total_test_samples:,}")
    print(f"False Positives Flagged: {fp_count:,}")
    print(f"Workload Percentage:     {review_workload_pct:.2f}% of traffic routed to manual review")
    print(f"Operational Verdict:     {'UNSUSTAINABLE (Exceeds capacity cap)' if fp_count > MAX_MANUAL_REVIEWS_CAP else 'FEASIBLE (Within capacity cap)'}")
...
def run_pipeline():
    print(f"\n{'=' * 70}\nABUSE-RING SENTINEL V10 (CAPACITY-CONSTRAINED)\n{'=' * 70}")
    ...
    # Run Diagnostics (Using X_tr_h.columns to match actual trained feature length)
    print_feature_importance(model_h, X_tr_h.columns)

    val_scores = validation_features["abuse_ring_score"].to_numpy()
    test_scores = test_features["abuse_ring_score"].to_numpy()
    evaluate_with_validation_check(model_h, X_va_h, y_val, val_amt, val_scores, X_te_h, y_test, test_amt, test_scores, thresh_h, validation_features, test_features)
    ...
    audit_operational_workload(res_h, len(test_df))
