#!/usr/bin/env python3
"""
ABUSE-RING SENTINEL - VERSION 9 (Diagnostic & Audited Release)
==============================================================
Upgrades:
    1. Feature Importance Audit: Inspects gain to detect target leakage from historical fraud rates.
    2. Validation vs. Test Overfit Check: Compares splits to verify generalization.
    3. Operational Workload Audit: Exposes the operational cost of high false-positive volumes.
"""
...
MODEL_FILE = "ieee_abuse_ring_sentinel_v9.pkl"
...
def evaluate_with_validation_check(model, X_val, y_val, val_amt, val_scores, X_test, y_test, test_amt, test_scores, threshold, features_val, features_test):
    val_res = calculate_cost_with_soft_boost(y_val, model.predict_proba(X_val)[:, 1], val_amt, threshold, features_val)
    val_recall = recall_score(y_val, val_res["adjusted_preds"], zero_division=0)
    
    test_res = calculate_cost_with_soft_boost(y_test, model.predict_proba(X_test)[:, 1], test_amt, threshold, features_test)
...
def audit_operational_workload(res_h, total_test_samples):
    ...
    print(f"Operational Verdict:     {'UNSUSTAINABLE (Requires secondary auto-triage)' if review_workload_pct > 30 else 'FEASIBLE'}")
...
def run_pipeline():
    print(f"\n{'=' * 70}\nABUSE-RING SENTINEL V9 (AUDITED RELEASE)\n{'=' * 70}")
    ...
    # Run Diagnostics
    print_feature_importance(model_h, HYBRID_FEATURES)
